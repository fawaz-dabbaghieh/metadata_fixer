...
#The table has column as keys, and rows as sample metadata
...
f_read = codecs.open("all_samples_after_json.tsv", encoding = "UTF-8-sig")
lines = f_read.readlines()
f_read.close()

#Checking which column is the DEEP_SAMPLE_ID column to name each file with its corrisponding DEEP_SAMPLE_ID
...
for i in range(1,len(lines)):
	file_name = lines[i].split("\t")[exp_id_idx] + "_smd.txt"
