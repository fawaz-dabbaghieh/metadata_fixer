	...
	if any("EXPERIMENT_ID" in s for s in lines):
		for idx,l in enumerate(lines):
			if l.startswith("EXPERIMENT_ID"):
				...
				file_name = re.sub("\_emd.tsv$", "", file_name) #removing _emd.tsv from the file name
...
if "emd.tsv" in all_files[1]:
	for f in all_files:
		f_read = codecs.open(f,encoding = "UTF-8-sig")
		f_write = open("after_python/"+f, "w+")
